...
These utilities keep image preparation consistent across classical vision,
training, and ML inference so shapes are not distorted by direct resizing.
"""
...
def composite_alpha_onto_background(
    ...
    Why this matters:
    - many downloaded PNG assets have transparent backgrounds
    - browsers render those over white, but naive OpenCV loads drop alpha and
      turn transparent pixels black
    - the vision and ML stages should process what the user actually sees
    """
    if image.ndim == 2:
        return image
