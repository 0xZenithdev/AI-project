...
class LinePairDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        mask_gray = cv2.resize(mask_gray, (self.width, self.height), interpolation=cv2.INTER_NEAREST)
...
def predict_line_mask(
    ...
    image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
    image_rgb = cv2.resize(image_rgb, (cfg.width, cfg.height), interpolation=cv2.INTER_AREA)
