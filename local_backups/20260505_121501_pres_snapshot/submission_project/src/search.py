"""Helpers used to clean paths, choose their order, and build drawing commands."""
...
def stitch_ordered_paths(
    ...
    for next_path in valid_paths[1:]:
        candidate = next_path
        if distance(current[-1], candidate[0]) > join_tolerance_mm:
            reversed_candidate = list(reversed(candidate))
            if distance(current[-1], reversed_candidate[0]) <= join_tolerance_mm:
        ...
        if distance(current[-1], candidate[0]) > 0.0:
            current.append(candidate[0])
        current.extend(candidate[1:])

    stitched.append(current)
    stats = PathStitchStats(
        input_paths=len(valid_paths),
        output_paths=len(stitched),
        merged_paths=len(valid_paths) - len(stitched),
    )
